...
export const chinaSite = defineType({
  ...
  description:"仅用于 www.camari.com.cn。不会影响英文或日文首页与联系方式。",
  ...
  ].map(field=>["home","about","downloads"].includes(field.name)?{...field,fields:("fields" in field?field.fields:[])?.map((child:{name:string})=>child.name==="status"?child:{...child,hidden:true})}:field),preview:{prepare:()=>({title:"中国站 · www.camari.com.cn"})}
});
