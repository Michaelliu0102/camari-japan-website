...
import productCopy from "./product-copy.json" with { type: "json" };
import faqCopy from "./faq-copy.json" with { type: "json" };
...
export function chineseCopy<T>(value:T):T {
  if(typeof value==='string') {
    ...
    const colorCount=value.match(/^(\d+)\s+colou?rs?$/i);
    if(colorCount)return `${colorCount[1]} 个颜色` as T;
