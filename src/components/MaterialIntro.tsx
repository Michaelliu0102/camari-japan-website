...
export function MaterialIntro({ locale, material }: MaterialIntroProps) {
  ...
  const localizedIntroTitle = material.introTitle[locale];
  const introTitle = locale === "ja" && material.slug === "alcantara"
    ? localizedIntroTitle.replace(/イタリアの技術と\s*/, "イタリアの技術と\n")
    : locale === "ja" && material.slug === "leather"
      ? localizedIntroTitle.replace(/職人の技が息づく\s*/, "職人の技が息づく\n")
      : localizedIntroTitle;
    ...
    <section className="bg-stone py-24 md:py-36" data-nav-invert>
      <div className="section-shell grid gap-16 md:grid-cols-12 md:items-stretch">
        <div className="md:col-span-5 md:self-center">
          <h2 className="whitespace-pre-line font-serif text-4xl leading-tight md:text-5xl">{introTitle}</h2>
