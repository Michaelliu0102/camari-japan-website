...
export interface NeonRevealProps {
  ...
  /** Explicit base, accent, and highlight colors for the neon light */
...
const NeonReveal: React.FC<NeonRevealProps> = ({
  ...
  useEffect(() => {
    ...
    const fallbackBaseColor = hslToRgb(color, 80, 60);
    const fallbackAccentColor = hslToRgb((color + 30) % 360, 70, 50);
    const fallbackHighlightColor = hslToRgb((color + 15) % 360, 75, 65);
    const [baseColor, accentColor, highlightColor] = colors
      ? colors.map((value) => new THREE.Color(value))
      : [
          new THREE.Color(fallbackBaseColor.r, fallbackBaseColor.g, fallbackBaseColor.b),
          new THREE.Color(fallbackAccentColor.r, fallbackAccentColor.g, fallbackAccentColor.b),
          new THREE.Color(fallbackHighlightColor.r, fallbackHighlightColor.g, fallbackHighlightColor.b),
        ];
