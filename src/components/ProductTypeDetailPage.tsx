...
import type { Material, ProductType, Sku } from "@/lib/content";
import { localizeBrandNames, type Locale } from "@/lib/locales";
import { buildBreadcrumbJsonLd, buildProductGroupJsonLd } from "@/lib/structured-data";
...
export function ProductTypeDetailPage({ initialSku, locale, material, productType, skus }: ProductTypeDetailPageProps) {
  ...
  const productGroupSchema = buildProductGroupJsonLd(siteConfig, {
    ...
    path: canonicalPath,
    image: productType.seo.image || initialSku.image,
    productGroupId: productType.productCode || `${material.slug}-${productType.slug}`,
    category: material.name[locale],
    locale,
    variants: skus.map((sku) => ({
      name: sku.colorName?.[locale]
        ? `${productType.name[locale]} — ${sku.colorName[locale]}`
        : `${productType.name[locale]} — ${sku.code}`,
      description: sku.summary[locale] || productType.summary[locale],
      path: `${canonicalPath}/${sku.slug}`,
      image: sku.image,
      sku: sku.code,
      color: sku.colorName?.[locale] || sku.code
    }))
  });
  ...
  return (
    <main>
      ...
      <SkuSwatches
        ...
        skus={skus}
      />
      <SpecificationTable locale={locale} productType={productType} sku={initialSku} />
